...
instance AWSRequest DeregisterType where
        ...
        response
          = receiveXMLWrapper "DeregisterTypeResult"
              (\ s h x ->
                 DeregisterTypeResponse' <$> (pure (fromEnum s)))

instance Hashable DeregisterType where

instance NFData DeregisterType where
        ...
        toQuery DeregisterType'{..}
          = mconcat
              ["Action" =: ("DeregisterType" :: ByteString),
               ...
               "VersionId" =: _dVersionId, "TypeName" =: _dTypeName,
               "Arn" =: _dARN, "Type" =: _dType]

-- | /See:/ 'deregisterTypeResponse' smart constructor.
newtype DeregisterTypeResponse =
  DeregisterTypeResponse'
...
instance NFData DeregisterTypeResponse where
