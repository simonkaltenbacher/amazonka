...
data SetTypeDefaultVersion =
  SetTypeDefaultVersion'
    { _stdvVersionId :: !(Maybe Text)
...
instance AWSRequest SetTypeDefaultVersion where
        type Rs SetTypeDefaultVersion =
             SetTypeDefaultVersionResponse
        request = postQuery cloudFormation
        response
          = receiveXMLWrapper "SetTypeDefaultVersionResult"
              (\ s h x ->
                 SetTypeDefaultVersionResponse' <$>
                   (pure (fromEnum s)))

instance Hashable SetTypeDefaultVersion where

instance NFData SetTypeDefaultVersion where
        ...
        toQuery SetTypeDefaultVersion'{..}
          = mconcat
              ["Action" =: ("SetTypeDefaultVersion" :: ByteString),
               ...
               "TypeName" =: _stdvTypeName, "Arn" =: _stdvARN,
               "Type" =: _stdvType]

-- | /See:/ 'setTypeDefaultVersionResponse' smart constructor.
newtype SetTypeDefaultVersionResponse =
  SetTypeDefaultVersionResponse'
    { _stdvrsResponseStatus :: Int
    }
  deriving (Eq, Read, Show, Data, Typeable, Generic)
...
setTypeDefaultVersionResponse
    :: Int -- ^ 'stdvrsResponseStatus'
    -> SetTypeDefaultVersionResponse
setTypeDefaultVersionResponse pResponseStatus_ =
  SetTypeDefaultVersionResponse' {_stdvrsResponseStatus = pResponseStatus_}
...
instance NFData SetTypeDefaultVersionResponse where
