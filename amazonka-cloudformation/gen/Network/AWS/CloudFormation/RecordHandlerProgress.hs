...
data RecordHandlerProgress =
  RecordHandlerProgress'
    { _rhpResourceModel          :: !(Maybe Text)
    , _rhpStatusMessage          :: !(Maybe Text)
    , _rhpErrorCode              :: !(Maybe HandlerErrorCode)
    , _rhpCurrentOperationStatus :: !(Maybe OperationStatus)
...
recordHandlerProgress
    :: Text -- ^ 'rhpBearerToken'
    -> OperationStatus -- ^ 'rhpOperationStatus'
    -> RecordHandlerProgress
...
instance AWSRequest RecordHandlerProgress where
        type Rs RecordHandlerProgress =
             RecordHandlerProgressResponse
        request = postQuery cloudFormation
        response
          = receiveXMLWrapper "RecordHandlerProgressResult"
              (\ s h x ->
                 RecordHandlerProgressResponse' <$>
                   (pure (fromEnum s)))

instance Hashable RecordHandlerProgress where

instance NFData RecordHandlerProgress where
...
newtype RecordHandlerProgressResponse =
  RecordHandlerProgressResponse'
    { _rhprsResponseStatus :: Int
    }
  deriving (Eq, Read, Show, Data, Typeable, Generic)
...
recordHandlerProgressResponse
    :: Int -- ^ 'rhprsResponseStatus'
    -> RecordHandlerProgressResponse
recordHandlerProgressResponse pResponseStatus_ =
  RecordHandlerProgressResponse' {_rhprsResponseStatus = pResponseStatus_}
...
instance NFData RecordHandlerProgressResponse where
