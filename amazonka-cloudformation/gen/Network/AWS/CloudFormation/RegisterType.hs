...
--
-- * 'rExecutionRoleARN' - The Amazon Resource Name (ARN) of the IAM execution role to use to register the type. If your resource type calls AWS APIs in any of its handlers, you must create an /<https:\/\/docs.aws.amazon.com\/IAM\/latest\/UserGuide\/id_roles.html IAM execution role> / that includes the necessary permissions to call those AWS APIs, and provision that execution role in your account. CloudFormation then assumes that execution role to provide your resource type with the appropriate credentials.
--
...
-- | The Amazon Resource Name (ARN) of the IAM execution role to use to register the type. If your resource type calls AWS APIs in any of its handlers, you must create an /<https:\/\/docs.aws.amazon.com\/IAM\/latest\/UserGuide\/id_roles.html IAM execution role> / that includes the necessary permissions to call those AWS APIs, and provision that execution role in your account. CloudFormation then assumes that execution role to provide your resource type with the appropriate credentials.
...
instance AWSRequest RegisterType where
        ...
        response
          = receiveXMLWrapper "RegisterTypeResult"
              (\ s h x ->
                 RegisterTypeResponse' <$>
                   (x .@? "RegistrationToken") <*> (pure (fromEnum s)))

instance Hashable RegisterType where

instance NFData RegisterType where
...
data RegisterTypeResponse =
  RegisterTypeResponse'
    { _rtrsRegistrationToken :: !(Maybe Text)
...
registerTypeResponse pResponseStatus_ =
  RegisterTypeResponse'
    {_rtrsRegistrationToken = Nothing, _rtrsResponseStatus = pResponseStatus_}
...
instance NFData RegisterTypeResponse where
